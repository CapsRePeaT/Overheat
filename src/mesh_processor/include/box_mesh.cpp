...
void round_boundary(DrawableTrimesh<>& mesh) {
	auto round_vec3d = [](auto vec) {
		vec3d ret;
		ret.x() = std::round(std::abs(vec.x()) * 100000.0) / 100000.0;
		ret.y() = std::round(std::abs(vec.y()) * 100000.0) / 100000.0;
		ret.z() = std::round(std::abs(vec.z()) * 100000.0) / 100000.0;
		return ret;
	};
...
BoxMesh::BoxMesh(const std::pair<float, float>& x1_x2_points,
	...
	diag_ = max_point.dist(min_point);
	layer = i_layer;
	++counter;
...
void BoxMesh::calculate_mesh() {
	...
	xz                   = trimeshFromPoly(xz_poly, diag_);
	...
	yz                   = trimeshFromPoly(yz_poly, diag_);
